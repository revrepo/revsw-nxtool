...
#            "%Y-%m-%dT%H:%M:%S+%:z"
    ...
    def demult_exception(self, event):
        ...
        for x in ['uri', 'server', 'proxy_name', 'block', 'learning', 'content', 'ip', 'date', 'var_name', 'country']:
            entry[x] = event.get(x, '')
        clean = entry
...
#        super(ESInject, self).__init__(value=20)
        ...
        self.index = self.cfg["elastic"]["index"]+datetime.datetime.today().strftime('-%Y.%m.%d') 
        # self.host = host
        # self.index = index
                    ...
                    body={
                        "events" : {
                            ...
                            "properties" : { "var_name" : {"type": "string", "index":"not_analyzed"},
                                        ...
                                        "learning" : {"type": "string", "index":"not_analyzed"},
